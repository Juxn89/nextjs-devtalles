...
const getPokemon = async (name: string): Promise<Pokemon> => {
	const res = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`, { cache: 'force-cache' });
...
export async function generateStaticParams() {
	...
	const pokemons = await first151Pokemons.json();

	return pokemons.results.map((pokemon: { name: string }) => ({ name: pokemon.name }));
}
